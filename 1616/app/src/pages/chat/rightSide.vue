    ...
    export default {
        ...
        methods: {
            ...
            getData(dataSourceId, tbMemberId){
                if(tbMemberId == "" || tbMemberId == undefined){
                    this.rsData = "";
                    return;
                }
                var data = {
                ...
                //右侧店铺信息地址
            //    var url = `http://154.8.222.129/front/evaluate/save`;
                ...
                .then(res => {
                    ...
                    console.log("this.rsData===",this.reData)
                    console.log("this.rsData.listProduct===",this.rsData.listProduct)
                    console.log("this.rsData.storeName",this.rsData.storeName)
                    let te = this.rsData.storeName
                    this.bus.$emit("test",te)
...
<template>
    <div class="rightSide" v-if="rsData">
        <div class="shop-info-box" v-if="!rsData.adDetail">
            <div class="shop-info">
                <div class="shop-name">
                    <a target="_blank" :href="rsData.shopDetailUrl"><img :src="rsData.storeLogo" :alt="rsData.storeName"></a>
                    <div>
                        <h2><a target="_blank" :href="rsData.shopDetailUrl"  :title="rsData.storeName">{{rsData.storeName}}</a></h2>
                        <div class="badge">
                            <i v-for="v of rsData.auditUrl" :key="v.code" :style="{background: `url(${v.identifyFlag})`}"></i>
                        </div>
                    </div>
                </div>
                <p class="enter_shop"><a target="_blank" :href="rsData.shopDetailUrl">{{$t("message.obj.enter_the_shop")}}</a></p>
                ...
                <p v-if="rsData.contactPhone">{{$t("message.obj.cellphone")}}：<em>{{rsData.contactPhone}}</em></p>
                <p v-if="rsData.contactTel">{{$t("message.obj.telephone")}}：<em>{{rsData.contactTel}}</em></p>
            </div>
...
<style scoped>
    ...
    .badge i{
        ...
        width: 20px;
        height: 20px;
        background: url('../../images/rihgt-icon.png') no-repeat 0 0;
    }
    .badge i.xin{
    ...
    .level i{
        ...
        height: 16px;
        margin-right: 3px;
        background: url('../../images/rihgt-icon.png') no-repeat -27px -30px;
